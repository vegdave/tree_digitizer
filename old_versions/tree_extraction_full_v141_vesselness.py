...
def branch_extraction_pipeline(gray, tip):
    ...
    # Normalise to 0-255 uint8 for downstream OpenCV operations
    p_high = np.percentile(vessel, 99.5)   # clip top 0.5% — removes needle body saturation, TODO check if this doesn't remove important features, i.e neeedle always brighter
    vessel_clipped = np.clip(vessel, 0, p_high) / p_high
    vessel_norm = (vessel_clipped * 255).astype(np.uint8)
    show_image("Sato Vesselness", vessel_norm, save_debug=True)
    
    """# Top-hat - Removes slowly-varying background haze that Sato can still pick up, TODO check if necessary 
    tophat_ksize = 31  # pixels; increase if background haze survives into threshold step
    tophat_kernel = cv2.getStructuringElement(
        cv2.MORPH_ELLIPSE, (tophat_ksize, tophat_ksize)
    )
    vessel_tophat = cv2.morphologyEx(vessel_norm, cv2.MORPH_TOPHAT, tophat_kernel)
    show_image("Vesselness Top-Hat", vessel_tophat)"""
    
    # Global threshold on the cleaned vesselness map
    _, thresh = cv2.threshold(vessel_norm, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)     # TODO: Otsu is adaptive per image but maybe not the best way, currently breaks some branches. See effects
    show_image("Vesselness Threshold", thresh, save_debug=True)

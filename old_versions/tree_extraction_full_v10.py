...
def branch_extraction_pipeline(gray, tip):
    gray_clahe = cv2.createCLAHE(clipLimit=7.0, tileGridSize=(15, 15)).apply(gray)
    show_image("CLAHE Grayscale", gray_clahe)

    # Blur to reduce noise before thresholding
    gray_blurred = cv2.GaussianBlur(gray_clahe, (15, 15), 0)
    show_image("Blurred Grayscale", gray_blurred)

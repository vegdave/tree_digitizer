...
def branch_extraction_pipeline(gray, tip): 
    ...
    gray = cv2.createCLAHE(clipLimit=10.0, tileGridSize=(13, 13)).apply(gray)
    ...
    """thresh = cv2.adaptiveThreshold(gray, 255,
        cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 91, 5)       # 71, 5 initially but needs fine tuning. Maybe denoising allows more aggressive parameters?
    show_image("Adaptive Threshold", thresh)"""

    # Binary threshold
    _, thresh = cv2.threshold(gray, 200, 255, cv2.THRESH_BINARY_INV)
    show_image("Binary thresh", thresh)

    ###### Opening/Closing to remove noise #######
    #TODO fine tune this        
    thresh = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3, 3)), iterations=2)
    show_image("Opened Threshold", thresh)
    
    #thresh = cv2.morphologyEx(thresh, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(7, 7)), iterations=1)
...
# Simple background subtraction using absolute difference. Can be improved with more advanced methods if needed (e.g. morphological background estimation)
...
if DENOISING:
    ...
    for path in image_paths[1:]:
        img = cv2.imread(path)
        diff = subtract_background(cv2.cvtColor(img, cv2.COLOR_BGR2GRAY), background)
        diffs.append(diff)

    # Average the images to enhance common features and reduce noise - floating dust. Can be either average or median, test other methods if ghosts or blurring is an issue
    combined = median_images(diffs)
    show_image("Combined Median Image", combined)

    # Noramlise and invert the background subtracted image to enhance contrast for needle tip detection
    combined = cv2.normalize(combined, None, 0, 255, cv2.NORM_MINMAX)
    combined = cv2.bitwise_not(combined) # Depending on the resulting colour of the branches TODO: Check why the differencing produces inverted colours

...
path = image_paths[17]  # Process the first image for testing
...
gamma_adjusted = adjust_gamma(gray, gamma=1.2)
show_image("Gamma Adjusted", gamma_adjusted, enable_show_images)

blur = cv2.medianBlur(gamma_adjusted, 5)
show_image("Median Blur", blur, enable_show_images)

clahe = cv2.createCLAHE(clipLimit=7.0, tileGridSize=(15, 15)).apply(blur)
show_image("CLAHE", clahe, enable_show_images)

thresh = cv2.adaptiveThreshold(clahe, 255,
    cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 31, 5)

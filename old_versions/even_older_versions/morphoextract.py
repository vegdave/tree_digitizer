...
for path in image_paths:
    ...
    show_image("Closed Mask", closing, enable_show_images)
    ...
    show_image("Tree", opening, enable_show_images)
    ...
    skeleton = skeletonize(reconnected/255).astype(np.uint8)

    skeleton_connected = bridge_endpoints_via_dilation(skeleton, max_bridge_dist=20)

    kernel = np.ones((2,2), np.uint8)
    skeleton_connected = cv2.dilate(skeleton_connected, kernel, iterations=1)
    show_image("Bridged Skeleton", skeleton_connected, enable_show_images)
  
    gray_clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8,8)).apply(gray)

    overlay = overlay_skeleton_on_gray(gray_clahe, skeleton_connected)

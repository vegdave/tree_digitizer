...
def local_contrast_stretch_percentile(gray, window_size=31, p_low=5, p_high=95):
    gray = gray.astype(np.float32)

    # Local percentiles
    low = percentile_filter(gray, p_low, size=window_size)
    high = percentile_filter(gray, p_high, size=window_size)

    # Stretch
    stretched = (gray - low) / (high - low + 1e-5)
...
def branch_extraction_pipeline(gray, tip):
    ...
    show_image("Blurred Grayscale", gray, save_debug=True)
    ...
    thresh = cv2.adaptiveThreshold(gray, 255,
        cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 251, 5)       # 71, 5 initially but needs fine tuning. Maybe denoising allows more aggressive parameters?
    show_image("Adaptive Threshold", thresh)
    
    ###### Opening/Closing to remove noise #######
    #TODO fine tune this        
    thresh = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(10, 10)), iterations=1)

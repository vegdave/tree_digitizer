...
# Local contrast stretching using percentiles in a sliding window
def global_contrast_stretch(gray, p_low=5, p_high=95):
    low = np.percentile(gray, p_low)
    high = np.percentile(gray, p_high)

    out = (gray - low) / (high - low + 1e-5)
    out = np.clip(out, 0, 1)
    return (out * 255).astype(np.uint8)


# Frangi vesselness filter to enhance tree branches before skeletonization
def frangi_enhance(gray, sigmas=range(1, 10), black_ridges=False):
    ...
    vesselness = frangi(
        gray,
        sigmas=range(6, 10),    # scale of branches
        black_ridges=False
    )

    vesselness = (vesselness - vesselness.min()) / (vesselness.max() - vesselness.min() + 1e-5)
...
def branch_extraction_pipeline(gray, tip):
    ...
    """vessel = frangi_enhance(gray, sigmas=range(1,6), black_ridges=True)
    show_image("Frangi Vesselness", vessel)"""

    # Contrast stretching using local percentiles
    # TODO fine tune percentiles
    norm = global_contrast_stretch(
        gray,
        p_low=5,
        p_high=95
    )
    show_image("Global Contrast", norm)
    ...
    # Filter connected components by size to remove small noise - fine tune this
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(thresh, connectivity=8)
    areas = stats[1:, cv2.CC_STAT_AREA]
    if areas.size > 0:
        median_area = np.median(areas)
        size_thresh = median_area * 0.7  # keep components larger than half the median area
        large_components = np.zeros_like(thresh)
        for i in range(1, num_labels):
            if stats[i, cv2.CC_STAT_AREA] >= size_thresh:
                large_components[labels == i] = 255
        thresh = large_components
        show_image("Size Filtered Threshold", thresh)    

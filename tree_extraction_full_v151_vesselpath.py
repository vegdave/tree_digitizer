...
# Find the longest path with BFS from the tip to any endpoint in the skeleton using BFS
def find_longest_path_from_tip(skeleton, tip):
    ...
    for node in queue:
        for nb in G.neighbors(node):
            if nb not in dist:
                dist[nb] = dist[node] + G[node][nb]['weight']
                pred[nb] = node
                queue.append(nb)
                if dist[nb] > dist[farthest]:
                    farthest = nb

    # Trace path back from farthest node to tip
    path = []
    cur = farthest
    ...
    print(f"Longest path from tip: {path_length} pixels | far_end=({farthest[1]}, {farthest[0]})")

    return path_mask, path_length
...
def branch_extraction_pipeline(gray, tip):
    ...
    # Crop to left of tip, exclude the needle body
    main_tree_left = main_tree.copy()
    main_tree_left[:, adjusted_tip[0]:] = 0

    # Find longest path from tip
    path_mask, path_length = find_longest_path_from_tip(main_tree_left, adjusted_tip)
    ...
    show_image("Longest Path Overlay", final_overlay, tip=adjusted_tip, save_debug=True)

    print(f"[{CURRENT_IMAGE}] longest path: {path_length} px")
...
if ENABLE_SHOW_IMAGES:
    print("Image display is ENABLED. Press any key on the image window to continue through each step.")
else:  
    print("Image display is DISABLED. Set ENABLE_SHOW_IMAGES=True to enable runtime visualisation.")
...
if BDG_SUB:
    ...
    for path in image_paths:
        ...
        skeleton = branch_extraction_pipeline(gray, tip)